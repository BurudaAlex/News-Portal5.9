...
class Author(models.Model):
    ...
    def update_rating(self):
        ...
        author_comment_rating=Comment.objects.filter(user_id=self.users).aggregate(
            comments_rating_sum=Coalesce(Sum('rating'), 0))
        author_post_comment_rating = Comment.objects.filter(post__author__name=self.users).aggregate(
            comments_rating_sum=Coalesce(Sum('rating'), 0))
